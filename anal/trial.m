    ...
    methods
        ...
        function write(obj, fid)
            ...
            fprintf(fid,'\n%d,%s,%d,%d,%d,%d,%.1f,%s,%d,%d,%.3f', obj.sub_id, obj.sub_sex, obj.sub_age, ...
                obj.block_id, obj.trial_id, obj.gain_on_left, obj.gamble_duration, gamblesStr,obj.sure_reward, obj.choose_sure, obj.reaction_time);
        end
...
%             end
            ...
            while GetSecs - timeStamp < display_info.fixation_duration + display_info.sure_reward_duration
                [~, ~, reactKey] = KbCheck;
                if any(reactKey)
            ...
            for i_gamble = 1:obj.n_gambles
                ...
                Screen('FrameRect', display_info.wPtr, display_info.rect_color, display_info.gamble_locus([1 2 1 2]).*display_info.window_rect([3 4 3 4]) + display_info.gamble_rect(1:4), display_info.rect_pen_width);
                Screen('FrameRect', display_info.wPtr, display_info.rect_color, display_info.gamble_locus([1 2 1 2]).*display_info.window_rect([3 4 3 4]) + display_info.gamble_rect(5:8), display_info.rect_pen_width);
                ...
                while GetSecs - timeStamp < display_info.fixation_duration + display_info.sure_reward_duration + i_gamble*obj.gamble_duration
                    [~, ~, reactKey] = KbCheck;
                    if any(reactKey)
                        if reactKey(KbName(display_info.sure_reward_key))
                            obj.choose_sure = 1;
            ...
            if ~reactTooEarly == 1
                ...
                Screen('Flip', display_info.wPtr, timeStamp + display_info.fixation_duration + display_info.sure_reward_duration + obj.n_gambles.*obj.gamble_duration, [], 1);
                while 1
                    [~, timeGetKey, reactKey] = KbCheck;
                    if  timeGetKey - timeStamp > display_info.time_limit + display_info.fixation_duration + display_info.sure_reward_duration + obj.n_gambles*obj.gamble_duration
                        obj.choose_sure = nan;
