...
trials = getSubData(subs);
for i = 1:length(trials)
    trials(i).gamble_duration = trials(i).block_id;
end
for i_sub = 1:length(subs)
    subTrials = trials([trials.sub_id] == subs(i_sub));
...
Meas = table([1 2 3 1 2 3]',categorical({'+', '+', '+', '-', '-', '-'})', 'VariableNames',{'time','sign'});
