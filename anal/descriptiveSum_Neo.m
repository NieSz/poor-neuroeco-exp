...
% meanUtilities = nanmedian(utilities,3);
...
%     plot(xRange,permute(utilities(:,i,:),[3,1,2]),'Color',lineColor{i},'LineWidth',1.5);
end
...
lh = nan(2,1);
for i = 1:2
    lh(i) = plot(xRange,medianUtilities(:,i),'Color',lineColor{i},'LineWidth',1.5);
    hold on
%     plot(xRange,permute(utilities(:,i,:),[3,1,2]),'Color',lineColor{i},'LineWidth',1.5);
end
line([-10, 10], [0, 0], 'Color','black','LineStyle','--');
...
ylabel('normal trial');
subplot(2,1,2)
bar(betas(length(subs)+1:end,:)');
xticklabels({'constant','sr+','sr-','gamble+','gamble-'});
ylabel('catch trial');

%% plot averaged
subplot(2,1,1)
bar(mean(betas(1:length(subs),:),1));
hold on
errorbar(1:5,mean(betas(1:length(subs),:),1),std(betas(1:length(subs),:),[],1)./sqrt(length(subs)),'.k','LineWidth',1.5);
xticklabels({'constant','sr+','sr-','gamble+','gamble-'});
% legend({'s1','s2','...'})
ylabel('normal trial');
subplot(2,1,2)
bar(mean(betas(length(subs)+1:end,:),1));
hold on
errorbar(1:5,mean(betas(length(subs)+1:end,:)),std(betas(length(subs)+1:end,:),[],1)./sqrt(length(subs)),'.k','LineWidth',1.5);
xticklabels({'constant','sr+','sr-','gamble+','gamble-'});
ylabel('catch trial');
...
%%
h = plot(1:9,mean(betas(1:length(subs),:))','LineWidth',2);
hold on
line([0,10],[0 0],'Color','black','LineStyle','--')
errorbar(1:9,mean(betas(1:length(subs),:),1),std(betas(1:length(subs),:),[],1)./sqrt(length(subs)),'.k','LineWidth',1.5);
% errorbar(0.75:0.5:5.25,[mean(betas(1:7,:));mean(betas(8:14,:))]',[std(betas(1:7,:),[],1);std(betas(8:14,:),[],1)]'./sqrt(7),'.k','LineWidth',1.5);
xticks([1:5 9])
xticklabels({'constant','sr','g1','g2','...','g7'});
ylabel('normal trial \beta s');
